# Source: https://github.com/SilvioGiancola/TrackingNet-devkit - TrackingNet github
# Modified in order to fit purposes of this project
# author: Max Zuo
# email: max.zuo@gmail.com
...
def evaluate(user_submission_filenames:list, target_annotation_filenames:list):
    ...
    for i, (user_submission, target_annotation) in tqdm(enumerate(zip(user_submission_filenames, target_annotation_filenames))):

        with open(target_annotation, 'r') as file:
            df = pd.read_csv(file, sep = ",", names = ["anno_x1","anno_y1","anno_w","anno_h"])

        with open(user_submission, 'r') as file_subm:
        ...
        Success_test_avg += np.array([np.sum(np.fromiter((i >= thres for i in df["IoU"]), dtype=np.bool)).astype(float) / (len(df["IoU"]))  for thres in Success_X])
        Precision_test_avg += np.array([np.sum(np.fromiter((i <= thres for i in df["distance"]), dtype=np.bool)).astype(float) / (len(df["distance"]))  for thres in Precision_X])
        NPrecision_test_avg += np.array([np.sum(np.fromiter((i <= thres for i in df["ndistance"]), dtype=np.bool)).astype(float) / (len(df["ndistance"]))  for thres in NPrecision_X])
    ...
    Precision = np.array(Precision_test_avg) / len(user_submission_filenames)
